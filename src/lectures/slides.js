...
function plotsToPanels() {
	const codePanels = document.querySelectorAll(".remark-slides-area .codePanel")
...
const live = function(){
  if (isLive) {
    const bodySlides = document.querySelectorAll(".remark-slide-content:not(.title-slide):not(.last-slide)")
		...
		for (let i = 0; i < panels.length; i++) {			
			...
			if (tabs.length == 2) {
				...
				if (names.every( (val, j, arr) => val === arr[0] )) {
					...
					slide = panels[i].parentElement.parentElement.parentElement.parentElement
					...
					if (!isCurrentSlide) {slide.classList.remove("remark-visible")}
				}
			}
		}
	}
...
addLogos = function() {
	const slides = document.querySelectorAll(".remark-slide-content:not(.last-slide)")
	let logo = document.createElement("a"),
		logoImg = document.createElement("img")	
	
	logo.classList.add("logo", "inst-logo")
	setAttributes(logo, {"href": "https://www.sussex.ac.uk/", "target": "_blank"})
	setAttributes(logoImg, {"src": "https://and.netlify.app/img/uos_paas.svg", "alt": "University of Sussex logo linked to the Uni website"})
	...
	for (let i = 0; i < slides.length; i++) {
		...
		setAttributes(logo, {"href": "https://paas.netlify.app", "target": "_blank"})
		setAttributes(logoImg, {"src": "https://and.netlify.app/img/paas_logo_lec.svg", "alt": "Analysing data logo linked to the website's main page"})
		logo.appendChild(logoImg)
		slides[i].appendChild(logo)
	}	
}
