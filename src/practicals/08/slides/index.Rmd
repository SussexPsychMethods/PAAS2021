...
## Getting started with of `dplyr`

In the coming weeks we'll be covering a package called `dplyr` to work with
**tibbles**
...
- `dplyr::group_by()` for **grouping** data

- `dplyr::summarise()` for **creating** summarises
]
...
You'll just replace the `...` the operation that you want to perform, and
`input_tibble` with the name of the tibble you want to perform the operations
on
...
We'll preview the `dplyr::select()` function this week because it's easiet

The `dplyr::select()` function is for **selecting** columns. To use it you just
need to give it a list of the columns you want
...
Returns the `island` and `sex` column
...
If you want to **drop** columns instead of **selecting** them then just add
  a `-` before the column name

```r
output_result <- dplyr::select(.data = penguins, -island)
```

Returns all the columns **except** for the `island` column
