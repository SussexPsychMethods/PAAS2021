...
expect when there are missing values (represented by `NA` in R) in the list. 
...
This problem is for people that want to try to implement the algorithm for a 
...
Once you've got it working, try to do it in one line of code by nesting the
...
# for numbers_with_missing we can't use length, because it'll count NAs
# but there is another way to count the values
# if we did following
# numbers_with_missing / numbers_with_missing
# we'd get a vector with a 1 everywhere there's isn't an NA and an NA otherwise
# then we can just work out the sum of that vector!
# making sure to include na.rm = TRUE, and tha's the length!

sqrt(sum((numbers_with_missing - mean(numbers_with_missing, na.rm = TRUE))^2, na.rm = TRUE) / (sum(numbers_with_missing / numbers_with_missing, na.rm = TRUE) - 1)) 

# Note the trick above won't work if some of the non-missing values are equal
# to 0, but there's an easy way to get around it. 
# sum((numbers_with_missing+1) / (numbers_with_missing+1), na.rm = TRUE) 
# that will give you the count of non-missing values even if one of the values
# is equal to zero
```
:::
