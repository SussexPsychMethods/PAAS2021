...
```{r class.source="copy"}
sc1_1 <- dplyr::filter(.data = starwars2, height < 70 ) # replace ___ with the correct code
...
```{r class.source="copy"}
dplyr::select(.data = sc1_1, name, height)  # replace ___ with the correct code
```
...
```{r class.source="copy"}
tidypenguins <- dplyr::select(.data = penguins2, species, island, sex, body_mass_g, year)
tidypenguins
```
...
```{r class.source="copy"}
tidypenguins_females <- dplyr::filter(.data = tidypenguins, sex == "female")
...
```{r class.source="copy"}
tidypenguins_females <- dplyr::mutate(.data = tidypenguins_females, mean_body_mass = mean(body_mass_g))
tidypenguins_females
```
...
```{r class.source="copy"}
tidypenguins_females <- dplyr::mutate(.data = tidypenguins_females, sd_body_mass = sd(body_mass_g))
tidypenguins_females
```
...
```{r class.source="copy"}
tidypenguins_females <- dplyr::mutate(.data = tidypenguins_females, z_score = (body_mass_g - mean_body_mass) / sd_body_mass)
tidypenguins_females
```
...
```{r class.source="copy"}
sc2_1 <- dplyr::mutate(.data = starwars2, bmi = height / mass)
```


```{r class.source="copy"}
sc2_2 <- dplyr::mutate(.data = sc2_1, max_bmi = max(bmi))

```

```{r class.source="copy"}
sc2_3 <- dplyr::filter(.data = sc2_2, bmi == max_bmi)
```
...
```{r class.source="copy"}
sc3_1 <- dplyr::select(.data = survey_data, id_code, gender, map, ldq, pdq)
```

```{r class.source="copy"} 
sc3_2 <- dplyr::filter(.data = sc3_1, gender == "M")
```

```{r class.source="copy"} 
dplyr::filter(.data = sc3_2, map < 40)
```
