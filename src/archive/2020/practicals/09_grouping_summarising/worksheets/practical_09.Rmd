...
stroop_data <- readr::read_csv("https://paas.netlify.app/archive/2020/practicals/09_grouping_summarising/worksheets/stroop_data.csv")
movie_data <- readr::read_csv("https://paas.netlify.app/archive/2020/practicals/09_grouping_summarising/worksheets/movies_data.csv")
...
```{r}
# Write answer here
sc1_1 <- dplyr::filter(.data = starwars2, ) %>% dplyr::select(.data = .,  ) 
sc1_1 
```
...
dplyr::group_by(.data = stroop_data, ) %>% dplyr::summarise(.data = ., )
```
...
You'll need to work out the aveages again, but first remove `sub01` from the data set.

```{r}
# Write answer here

dplyr::filter(.data = stroop_data, ) %>% dplyr::group_by() %>% dplyr::summarise()
...
dplyr::group_by(.data = penguins, ) %>% summarise()
```
